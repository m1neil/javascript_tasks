...
const totalPrice = getTotalSum(dataList, 'price')
outputResult('Вартість', totalPrice, '.list-page__item--1')

// 2)Знайти кількість товарів, у яких ціна зменшилась(price < old_price).
const goodsWithChangePrice = dataList.reduce((prevCount, { price, old_price }) =>
	price < old_price ? prevCount + 1 : prevCount, 0)
outputResult('Кількість:', goodsWithChangePrice, '.list-page__item--2')

// 3) Товари, які доступні(sell_status: "available")
const goodsAvailable = dataList.filter(({ sell_status }) => sell_status === 'available')
outputResult('Товари:', goodsAvailable, '.list-page__item--3', true)
...
const modifyGoodsAvailable = goodsAvailable.map(({ id, price, old_price, usd_price }) => (
	{ id, price, old_price, usd_price }
))
outputResult('Товари:', modifyGoodsAvailable, '.list-page__item--4', true)
